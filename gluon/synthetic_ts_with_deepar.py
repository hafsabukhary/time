...
gluonts_datafolder='gluonts_data/m5_dataset'
dataset_path=Path(gluonts_datafolder)
df_timeseries,df_dynamic,df_static = generate_dataframes(dataset_path)
...
calendar               = pd.read_csv(data_folder+'/calendar.csv')[0:10]
sales_train_val        = pd.read_csv(data_folder+'/sales_train_validation.csv.zip')[0:10]
sample_submission      = pd.read_csv(data_folder+'/sample_submission.csv.zip')[0:10]
sell_prices            = pd.read_csv(data_folder+'/sell_prices.csv.zip')[0:10]
...
df_timeseries1 = sales_train_val[ cols_timeseries]
df_dynamic1    = calendar[ cols_calendar]
df_static1     = sales_train_val[ cols_static]

print(df_timeseries1.head(10))
print(df_dynamic1.head(10))
print(df_static1.head(10))
...
dates = [ startdate for _ in range(len(sales_train_val))]



##### Params 3 : Calendar features : Dynamic categories cols
...
##### Params 4 : Static Features : categories product, shop
"""
state_id           = (sales_train_val["state_id"].astype("category").cat.codes.values)
store_id           = (sales_train_val["store_id"].astype("category").cat.codes.values)
cat_id             = (sales_train_val["cat_id"].astype("category").cat.codes.values)
dept_id            = (sales_train_val["dept_id"].astype("category").cat.codes.values)
item_id            = (sales_train_val["item_id"].astype("category").cat.codes.values)
stat_cat_list       = [item_id, dept_id, cat_id, store_id, state_id]
"""


cols_static_cat = ["item_id", "dept_id", "cat_id", "store_id", "state_id"] 

for x in  cols_static_cat :
    globals()[x] = (sales_train_val[x].astype("category").cat.codes.values)
stat_cat_list       = [   globals()[x]  for x in cols_static_cat    ]


stat_cat            = np.concatenate(stat_cat_list)
stat_cat            = stat_cat.reshape(len(stat_cat_list), len(item_id)).T


##############################################################
##### Model definiton  #######################################
"""
state_id_un        = np.unique(state_id)
cat_id_un          = np.unique(cat_id)

store_id_un        = np.unique(store_id)
dept_id_un         = np.unique(dept_id)
item_id_un         = np.unique(item_id)
cardinalities = [len(item_id_un), len(dept_id_un), len(cat_id_un), len(store_id_un), len(state_id_un), ]
"""

cardinalities = [len( np.unique(globals()[x]) )  for x in cols_static_cat ]
...
#create data
def create_timeseries1d(start_date = '1/1/2000' ,end_date = None ,periods = 720, freq = 'D' ,weight = 1 , t=None,expression = 'math.cos(index)*0.02' ):
  import math
  import pandas as pd
