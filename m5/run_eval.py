...
def features_generate_file(dir_in, dir_out, my_fun_features, features_group_name, input_raw_path = None, max_rows = 10, step_wise_saving = False) :
    ...
    dfnew, cat_cols= my_fun_features(merged_df, input_raw_path, dir_out, features_group_name, max_rows)
...
def raw_merged_df(input_path="data/", out_path='out/',  max_rows=10):
    df_sales_train            = pd.read_csv(input_path + "/sales_train_eval.csv")
    df_calendar               = pd.read_csv(input_path + "/calendar.csv")
    df_sales_val              = pd.read_csv(input_path + "/sales_train_val.csv")
    df_sell_price             = pd.read_csv(input_path + "/sell_prices.csv")
    ...
    if not max_rows == -1:
        ...
        df_sales_val_melt         = pd.melt(df_sales_val, id_vars = ['id', 'item_id', 'dept_id', 'cat_id', 'store_id', 'state_id'], var_name = 'day', value_name = 'demand')
...
def run_eval(input_path, max_rows = None, n_experiments = 3):
    ...
    for ii in range(n_experiments):
        ...
        df          		 = load_data(input_path, cols_cat + cols_num)
...
def generate_feature_all( input_path = "data/output", out_path="." , input_raw_path = ".", max_rows = 10):
    features_generate_file(".", input_path, features_time_basic, "basic_time")
    features_generate_file(".", input_path, features_rolling, "rolling")
    features_generate_file(".", input_path, features_lag, "lag")
    features_generate_file(".", input_path, features_tsfresh, "tsfresh", input_raw_path, max_rows, step_wise_saving = True)
...
def main( input_path = "data/output", out_path="",
    ...
    if do_generate_raw :
      raw_merged_df(input_path= input_path, out_path=out_path, max_rows = max_rows)

    if do_generate_feature :
      generate_feature_all(input_path="data/output", out_path="", input_raw_path = input_path, max_rows = max_rows)


    run_eval(input_path="data/output")
